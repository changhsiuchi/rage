...
function analyzePost(text) {
  ...
  while (i < text.length) {
    ...
    if (found && found.word.length > 1) {  // require length >= 2 to avoid single-char noise
...
function importCSV(e) {
  ...
  reader.onload = ev => {
    ...
    try {
      ...
      if (lines.length < 2) throw new Error('CSV 格式錯誤');
      const headers = parseCsvLine(lines[0]);
      const idx = name => headers.findIndex(h => h.toLowerCase().includes(name));
      const newPosts = [];
      for (let i = 1; i < lines.length; i++) {
        const cols = parseCsvLine(lines[i]);
        const content = cols[idx('content')] || cols[idx('貼文')] || '';
        if (!content) continue;
        newPosts.push({
          id: Date.now() + i,
          content,
          likes: parseInt(cols[idx('like')] || cols[idx('讚')] || 0) || 0,
          comments: parseInt(cols[idx('comment')] || cols[idx('留言')] || 0) || 0,
          shares: parseInt(cols[idx('share')] || cols[idx('分享')] || 0) || 0,
          date: cols[idx('date')] || cols[idx('日期')] || '',
          account: cols[idx('account')] || cols[idx('帳號')] || '',
          topic: cols[idx('topic')] || cols[idx('議題')] || '',
          url: cols[idx('url')] || '',
          createdAt: new Date().toISOString(),
        });
      }
